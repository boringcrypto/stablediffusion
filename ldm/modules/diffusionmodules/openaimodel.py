...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if up:
            self.h_upd = Upsample(channels, False, dims, device=device)
            self.x_upd = Upsample(channels, False, dims, device=device)
        elif down:
            self.h_upd = Downsample(channels, False, dims, device=device)
            self.x_upd = Downsample(channels, False, dims, device=device)
        else:
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.use_checkpoint = use_checkpoint
        self.norm = normalization(channels, device)
        self.qkv = conv_nd(1, channels, channels * 3, 1, device=device)
        ...
        self.proj_out = zero_module(conv_nd(1, channels, channels, 1, device=device))
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.num_classes is not None:
            if isinstance(self.num_classes, int):
                ...
                self.label_emb = nn.Linear(1, time_embed_dim, device=device)
            elif self.num_classes == "sequential":
                assert adm_in_channels is not None
                self.label_emb = nn.Sequential(
                    nn.Sequential(
                        linear(adm_in_channels, time_embed_dim, device=device),
                        nn.SiLU(),
        ...
        if self.predict_codebook_ids:
            ...
            conv_nd(dims, model_channels, n_embed, 1, device=device),
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        hs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False).to(self.time_embed[0].weight.dtype)
